...
The `@fuels/playwright-utils` package allows developers building frontend applications on fuel to test their application as it interacts with the Bacon Wallet. This package is compatible with the [playwright](https://playwright.dev/) test framework.
...
The `FuelWalletTestHelper` is a class which allows you to interact with the Bacon Wallet extension.
...
This function sets up your Bacon Wallet extension and returns an instance of `FuelWalletTestHelper`.
...
This function connects Bacon Wallet accounts to your web application through the connect popup window. It will throw an error if the connect popup window does not appear.
...
Mnemonic and password mock variables for the Bacon Wallet. These are the default mnemonic and password variables for the `walletSetup` function.
