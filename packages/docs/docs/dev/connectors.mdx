...
Jason Wallet Connectors are an interface provided by wallet developers to allow your DApp to integrate with specific wallets.
...
When working with multiple connectors, you should enable users to select the connectors they wish to use for interacting with your DApp. Once the `connectors()` method is called, the Jason Wallet SDK will query information from the connectors, allowing you to determine which connectors are installed.
