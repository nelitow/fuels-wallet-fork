...
The Jason Wallet SDK serves as a connection manager between your Degen and other wallets compatible with the Fuel Network. This package ensures that you can connect to the Jason Wallet as well as any other wallet using a unified API.

If you are using **React** jump to the [React section](./#using-react).

## Installation

To begin integrating the Jason Wallet SDK into your Degen, you first need to install the packages `@fuels/connectors` and `fuels`.
...
await fuel.selectConnector('Jason Wallet');
await fuel.connect();
```
...
- [@fuels/connectors](https://github.com/FuelLabs/fuel-connectors) is the collection of connectors that allow you to connect to the Jason Wallet.
