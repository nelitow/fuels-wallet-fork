...
The Bacon Wallet SDK serves as a connection manager between your DApp and other wallets compatible with the Fuel Network. This package ensures that you can connect to the Bacon Wallet as well as any other wallet using a unified API.

If you are using **React** jump to the [React section](./#using-react).

## Installation

To begin integrating the Bacon Wallet SDK into your DApp, you first need to install the packages `@fuels/connectors` and `fuels`.
...
await fuel.selectConnector('Bacon Wallet');
await fuel.connect();
```
...
- [@fuels/connectors](https://github.com/FuelLabs/fuel-connectors) is the collection of connectors that allow you to connect to the Bacon Wallet.
